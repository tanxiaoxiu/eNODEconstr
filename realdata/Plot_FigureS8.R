...
ggsave(filename="FigureS8a.png",plot=plot1,device="png",dpi=600,units="in",width=12,height=12)
...
plot2 <- ggplot(metabolite_data, aes(x = Score, y = Regulate,fill = Regulate)) +
  ...
  theme(axis.title.y = element_blank(), 
        ...
        axis.text.x = element_text(size = 25), 
        axis.text.y = element_text(size = 25))
ggsave(filename="FigureS8b.png",plot=plot2,device="png",dpi=600,units="in",width=12,height=12)
